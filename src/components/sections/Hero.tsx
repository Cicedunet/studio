...
import { 
  Menu, Sparkles, Weight, Instagram, Facebook, 
  MessageCircle, Briefcase, Gift, Phone, List, 
  ShoppingBag, HeartPulse, Droplets 
} from "lucide-react";
...
interface NavLinkItem {
  href?: string;
  label: string;
  icon: React.ElementType; // Utilise ElementType pour les icônes Lucide
  subLinks?: SubLink[];
}
...
const socialLinks = [
  ...
  { 
    href: "https://www.tiktok.com/@lizfrancine_elegance", 
    label: "TikTok", 
    icon: () => (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6">
        ...
        <path d="M11.5 16.5v-11" />
        <path d="m7.5 11.5 8-4" />
      </svg>
    ) 
  },
                      ...
                      {[...productLinks, ...navLinks].map((link) => (
                        link.subLinks ? (
                          ...
                          <SheetClose asChild key={link.label}>
                            <Link
                              href={link.href || "#"}
                              className="transition-colors hover:text-primary flex items-center gap-2 py-2"
                            >
                              <link.icon className="h-5 w-5" />
                              {link.label}
                            </Link>
        ...
        <div className="grid items-center gap-8 md:grid-cols-2 lg:gap-12">
          <div className="flex flex-col items-start space-y-6 animate-in fade-in slide-in-from-left-8 duration-1000 delay-200">
            ...
            <p className="text-lg text-muted-foreground md:text-xl">
              Je suis JOSCAAD53, maman, professionnelle des chiffres et distributrice de parfums haut de gamme. Passionnée par le bien-être et l'élégance, je partage ici des produits et astuces qui vont ravir ton quotidien.
              <br/><br/>
              Je t'aide également à construire ton propre business en toute liberté avec des conseils à la clé. Explore mon univers raffiné.
            </p>
              ...
              {socialLinks.map((social, index) => {
                ...
                return (
                  <a 
                    ...
                    className="text-muted-foreground transition-all duration-300 hover:text-primary hover:scale-125 hover:rotate-6 animate-in fade-in zoom-in-50 duration-500" 
          ...
          <div className="flex justify-center items-center p-6 row-start-1 md:row-start-auto animate-in fade-in slide-in-from-right-8 duration-1000 delay-200">
            <Image
              src="https://static3.depositphotos.com/1000859/118/i/450/depositphotos_1180794-stock-photo-perfume-bottle-with-crystals.jpg"
              alt="Flacon de parfum élégant"
              width={350}
              height={350}
